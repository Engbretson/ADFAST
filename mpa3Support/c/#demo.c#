// --------------------------------------------------------------------------
// DEMO.C : DMPA3.DLL Software driver C example
// --------------------------------------------------------------------------
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void PrintData(long HUGE *dat, short nDev)
{
  short index;
...
int run(char *command)
{
  ...
  if (!stricmp(command, "?")) help();
    ...
    err = (*lpDat)(dat, 0);
    if (nDev) PrintData(dat,nDev);
    else PrintData(dat,1024);
  }
  else if (!stricmp(command,"H")) {
    (*lpHalt)(0);
  }
  else if(!strnicmp(command, "ADC=", 4)) {
    sscanf(command+4, "%hd", &nDev);
    (*lpRun)(0, command);
  }
